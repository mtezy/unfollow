...
checkdepeden(){
	...
	cekdep=$(dpkg -l | grep -o 'curl\|grep' | uniq)	if [[ ! $cekdep =~ "curl" ]]
...
unfoll(){
	...
	then
		...
		printf "${merah}[+]${NC} Can't Unfollow $followedname [Sleeping 1sec]\n"
